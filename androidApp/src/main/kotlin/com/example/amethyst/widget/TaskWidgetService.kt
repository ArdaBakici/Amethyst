...
class TaskWidgetViewsFactory(private val context: Context) : RemoteViewsService.RemoteViewsFactory {
    ...
    override fun getViewAt(position: Int): RemoteViews {
        ...
        try {
            ...
            // Set click intent for task content area (opens detail page)
            val contentFillInIntent = Intent().apply {
                putExtra("task_id", task.id)
                putExtra("action", "open_task")
            }
            views.setOnClickFillInIntent(R.id.task_content, contentFillInIntent)

            // Set click intent for checkbox (marks task as done)
            val checkboxFillInIntent = Intent().apply {
                putExtra("task_id", task.id)
                putExtra("action", "complete_task")
            }
            views.setOnClickFillInIntent(R.id.task_checkbox, checkboxFillInIntent)
        } catch (e: Exception) {
